...
function onDayClickAction(wideView, date, clickel, instance) {
	...
	if(wideView==1) {
		...
		$$('#cal_event_'+instance+' .calendarBody')[0].getElements('div[class$=sel]').each(function(el) {
				 el.setProperty('class', el.getProperty('class').replace('_sel', ''));
				});
		clickel.setProperty('class', clickel.getProperty('class')+'_sel');
		ajaxRequest('post', 'index.php?pt['+instance+'-renderLeft]', 'date='+date+'&ctg='+ctg, $$('#cal_event_'+instance+' td[class=calendarLeft]')[0], {'script':true, 'cache':true, 'callback':updateTooltips});
	}
	else {
		if(layer_open) return false;
		$$('#cal_event_'+instance+' .calendarBody')[0].getElements('div[class^=day]').each(function(el) {el.setStyle('position', 'static')});
